...
function calc_best_question()
{
    ...
    result_str = "推薦數字: " + best_q + ", Score: " + best.toFixed(2);
    var count = 0;
    for (var i = 0; i < potential.length; i++)
    {
        ...
        if (best_info[key] != undefined)
        {
            ...
            result_str += "\n" + result_formatter(s, b) + ": 可能性: " + best_info[key] + ", 概率: " + probability.toFixed(2) + " % ";
        }
    }
    if (count <= 1)
    {
        mainForm.txt_answer.value = "游戲終了! 答案: " + best_q + "\n點擊重新開始進行新一輪游戲!";
        mainForm.input_S.value = "4";
    }
    else
    {
...
function reset_game()
{
    mainForm.input_Q.value = getRandomarr();
    mainForm.input_S.value = "";
    mainForm.input_B.value = "";
